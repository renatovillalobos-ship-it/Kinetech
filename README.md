...
3. Instalación de recursos necesarios para la utilización de la aplicación:
  Para instalar las librerías existen 2 formas para uno de los casos debes cambiar de carpeta ya sea utilizando el comando cd.. o copiando nuevamente la ruta a donde quieres moverte, en este caso debes ubicarte a la altura del archivo manage.py:
    ...
    b)De forma manual: Aqui puedes escribirlas donde quieras porque el comando pip install instala las librerias en el entorno de python que estés usando,las instalaciones deben realizarse en Símbolo del Sistema (CMD) en el siguiente orden:
...
        'PASSWORD': '<password>', #Tu contrasena de PostgreSQL
        'HOST': 'localhost',
...
#Recordar eliminar los <> dentro de DATABASE para evitar conflictos. | Al copiar y pegar puede que de errores por los espacios, revisar eso y en caso de que desee dejarlo ordenado utilizar el TAB.
...
5. Aplicamos migraciones: los comandos estarán listado por orden de utilización, estos de deben de utilizar uno por uno dentro del Símbolo del Sistema (cmd).
        ...
        II. python manage.py migrate --fake Estudiante
